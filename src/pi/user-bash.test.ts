...
function fakeContext(cwd = "/tmp/pi-yaml-hooks-test"): ExtensionContext {
...
async function withEnabledHandler<T>(
  ...
  const tempDir = mkdtempSync(path.join(os.tmpdir(), "pi-yaml-hooks-userbash-"))
