    ...
    try:
        ...
        for event in pending:
            ...
            if int(event["branch_generation"]) != int(ctx["branch_generation"]):
                # Pair publish_state + capture_events writes in one
                # transaction so a crash between them cannot leave the
                # row marked ``blocked_conflict`` while publish_state
                # still references the previous commit_oid.
                with transaction(conn):
                    update_publish_state(
                        conn,
                    ...
                    conn.execute(
                        "UPDATE capture_events SET state='blocked_conflict', error=? WHERE seq=?",
                        ("stale branch generation", int(event["seq"])),
                    )
                processed += 1
                continue
            try:
